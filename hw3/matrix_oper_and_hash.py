...
class Matrix:
    ...
    def hash_counter(self):
        seed = 42
        max_num = max(max(row) for row in self.data) # max value in matrix
        length_hash = len(str(max_num**3 - max_num*2 + max_num*seed)) # max length of hash-value
        hash_values = ''

        for row in self.data:
            for value in row:
                sq_val = value**3 - value*2 + value*seed
                val_str = str(sq_val).zfill(length_hash) # fill in the missing values on the left with zeros
                hash_values += val_str

        return hash_values, length_hash          
